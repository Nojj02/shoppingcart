...
public class DomainEventToDtoMapper {
    public static DomainMessage map(EventRecord eventRecord) throws IOException {
    ...
    public static Collection<DomainMessage> map(Collection<EventRecord> eventRecords) {
        return eventRecords.stream()
                .map(eventRecord -> {
                    try {
                        return DomainEventToDtoMapper.map(eventRecord);
                    } catch (IOException e) {
                        e.printStackTrace();
                        return null;
                    }
